*** Settings ***
Resource        ../_resources/palavras.resource
...
Foreign Extractors
    ...
    [Teardown]    Clean Up After Working with File and Settings    ${chave}

Code Overrides
    ${chave} =    Set Variable    Code overrides.ipynb
    Setup Notebook    Python    ${chave}
    ${virtual_path} =    Set Variable    ${VIRTUALDOCS DIR}${/}${chave}
...
File Content Should Be Equal
    [Arguments]    ${chave}    ${text}
    Wait Until Keyword Succeeds    5x    1s    File Should Not Be Empty    ${chave}
    ${document} =    Get File    ${chave}
